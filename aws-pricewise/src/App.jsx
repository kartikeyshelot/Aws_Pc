...
function ec2Cost(c, r) { const ins = getEC2Instances(r); if (!ins[c.instance]) return 0; const p = ins[c.instance]?.prices?.[c.os] ?? null; return p != null ? p * c.count * c.hours : 0; }
...
function rdsCost(c, r) { const ins = getRDSInstances(r); if (!ins[c.instance]) return 0; const h = ins[c.instance]?.prices?.[c.engine]; if (h == null) return 0; const m = c.multiAZ ? 2 : 1; const sp = getRDSStorage(r); return h * HOURS_PER_MONTH * m + (sp ? c.storageGB * sp * m : 0); }
...
function auroraCost(c, r) { const p = getAuroraPricing(r); if (!p?.instances || !p.instances[c.instance]) return 0; const h = p.instances[c.instance].prices?.[c.engine]; if (h == null) return 0; return h * HOURS_PER_MONTH * c.replicas + c.storageGB * (p.storagePerGB || 0) + (c.ioMillions || 0) * (p.ioPerMillion || 0); }
...
function elasticacheCost(c, r) { const nodes = getElastiCacheNodes(r); if (!nodes[c.nodeType]) return 0; return (nodes[c.nodeType]?.price || 0) * HOURS_PER_MONTH * c.numNodes; }
...
// ═══════════════════════════════════════════════════════════════════
// EC2 INSTANCE COMPARE (head-to-head)
// ═══════════════════════════════════════════════════════════════════
function EC2InstanceCompare({ region, currentConfig, onSelectInstance }) {
  const ins = getEC2Instances(region);
  const types = Object.keys(ins).sort((a, b) => {
    const f = x => x.split(".")[0];
    const sz = x => SIZE_ORDER.indexOf(x.split(".")[1]);
  ...
  const [instA, setInstA] = useState(currentConfig?.instance || types[0] || "");
  const [instB, setInstB] = useState(() => {
    const a = currentConfig?.instance || types[0];
    return types.find(t => t !== a) || types[0] || "";
  });
  const [os, setOs] = useState(currentConfig?.os || "linux");

  if (!types.length) return <NotAvailable message={"No EC2 data for " + region} />;

  const dataA = ins[instA];
  const dataB = ins[instB];
  const priceA = dataA?.prices?.[os] ?? null;
  const priceB = dataB?.prices?.[os] ?? null;

  const vcpuWin  = dataA && dataB ? (dataA.vcpu > dataB.vcpu ? "A" : dataB.vcpu > dataA.vcpu ? "B" : null) : null;
  const ramWin   = dataA && dataB ? (dataA.memory > dataB.memory ? "A" : dataB.memory > dataA.memory ? "B" : null) : null;
  const priceWin = priceA !== null && priceB !== null ? (priceA < priceB ? "A" : priceB < priceA ? "B" : null) : null;
  const saving   = priceA !== null && priceB !== null ? Math.abs(priceA - priceB) : null;
  const pctDiff  = saving !== null && priceA && priceB ? (saving / Math.min(priceA, priceB)) * 100 : null;

  const instanceOptions = types.map(t => ({ value: t, label: t + "  (" + ins[t].vcpu + " vCPU, " + ins[t].memory + " GiB)" }));

  return (
    <div className="ic-wrap">
      <div className="ic-selectors">
        <div className="ic-slot">
          <SearchSelect label="Instance A" value={instA} onChange={setInstA} options={instanceOptions} placeholder="Search instance..." />
          <button className="ic-use-btn" onClick={() => onSelectInstance(instA, os)}>Use A</button>
        </div>
        <div className="ic-vs">VS</div>
        <div className="ic-slot">
          <SearchSelect label="Instance B" value={instB} onChange={setInstB} options={instanceOptions} placeholder="Search instance..." />
          <button className="ic-use-btn" onClick={() => onSelectInstance(instB, os)}>Use B</button>
        </div>
      </div>

      <div className="ic-os-bar">
        <span className="ic-os-lbl">OS</span>
        <div className="fc-os-group">
          {OS_OPTIONS.map(o => (
            <button key={o.value} className={"fc-os" + (o.value === os ? " active" : "")} onClick={() => setOs(o.value)}>{o.label}</button>
          ))}
        </div>
      </div>
      ...
      <div className="ic-table-wrap">
        <table className="ic-table">
          <thead>
            <tr>
              <th className="ic-th-row"></th>
              <th className="ic-th-inst ic-col-a">{instA}</th>
              <th className="ic-th-inst ic-col-b">{instB}</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td className="ic-lbl">vCPU</td>
              <td className={"ic-val" + (vcpuWin === "A" ? " ic-win" : "")}>{dataA?.vcpu ?? "—"}</td>
              <td className={"ic-val" + (vcpuWin === "B" ? " ic-win" : "")}>{dataB?.vcpu ?? "—"}</td>
            </tr>
            <tr>
              <td className="ic-lbl">RAM (GiB)</td>
              <td className={"ic-val" + (ramWin === "A" ? " ic-win" : "")}>{dataA?.memory ?? "—"}</td>
              <td className={"ic-val" + (ramWin === "B" ? " ic-win" : "")}>{dataB?.memory ?? "—"}</td>
            </tr>
            <tr>
              <td className="ic-lbl">Per hour</td>
              <td className={"ic-val ic-price" + (priceWin === "A" ? " ic-win" : "")}>
                {priceA !== null ? fmt(priceA) : <span className="ic-na">N/A</span>}
              </td>
              <td className={"ic-val ic-price" + (priceWin === "B" ? " ic-win" : "")}>
                {priceB !== null ? fmt(priceB) : <span className="ic-na">N/A</span>}
              </td>
            </tr>
            <tr>
              <td className="ic-lbl">Per month</td>
              <td className={"ic-val ic-price" + (priceWin === "A" ? " ic-win" : "")}>
                {priceA !== null ? fmt(priceA * 730) : <span className="ic-na">N/A</span>}
              </td>
              <td className={"ic-val ic-price" + (priceWin === "B" ? " ic-win" : "")}>
                {priceB !== null ? fmt(priceB * 730) : <span className="ic-na">N/A</span>}
              </td>
            </tr>
            {saving !== null && pctDiff !== null && priceWin !== null && (
              <tr className="ic-saving-row">
                <td className="ic-lbl">Savings</td>
                <td colSpan={2} className="ic-saving">
                  <strong>{priceWin === "A" ? instA : instB}</strong> is {pctDiff.toFixed(1)}% cheaper
                  <span className="ic-saving-amt"> · saves {fmt(saving)}/hr · {fmt(saving * 730)}/mo</span>
                </td>
              </tr>
            )}
          </tbody>
      ...
      <div className="fc-hint">Click "Use A" or "Use B" to load that instance into the calculator</div>
    </div>
  );
}
            ...
            <>
              ...
              <div className="pw-main-hd">
                ...
                <div className="pw-main-hd-right">
                  ...
                  {active === "ec2" && (
                    <button className={"view-btn" + (view === "compare" ? " active" : "")} onClick={() => setView(v => v === "compare" ? "config" : "compare")}>
                      <svg viewBox="0 0 16 16" width="12" height="12" fill="none" stroke="currentColor" strokeWidth="1.5">
                        <path d="M1 4h6M1 8h6M1 12h6M9 4h6M9 8h6M9 12h6" />
                      </svg>
                      Compare
                    </button>
                  )}
